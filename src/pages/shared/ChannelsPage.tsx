    ...
    <PageShell title={title} description={description} eyebrow={eyebrow}>
      <section className="grid gap-4 lg:grid-cols-2">
        {channels.map((channel) => (
          <article key={channel.id} className="surface-card p-5">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold text-slate-900">{channel.name}</h3>
              <StatusBadge status={channel.status} />
            </div>

            <p className="text-sm text-slate-600">{channel.audience}</p>
            ...
            <div className="mt-4 space-y-2 text-sm text-slate-600">
              ...
              <p className="flex items-center gap-2">
                <MessageCircleMore className="h-4 w-4 text-petrol-700" />
